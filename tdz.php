...
class tdz
{
    ...
    public static
        ...
        'mpe' => 'video/mpeg',
        'mov' => 'video/quicktime',
        'swf' => 'video/quicktime',//@todo duplicated
        '3gp' => 'video/quicktime',
        'm4a' => 'video/quicktime',
        'aac' => 'video/quicktime',
        'm3u' => 'video/quicktime',
        ...
        'ttf' => 'font/ttf',//@todo duplicated
        'woff' => 'font/woff',//@todo duplicated
        'woff2' => 'font/woff2',//@todo duplicated
        'eot' => 'font/vnd.ms-fontobject',//@todo duplicated
    ),
        $browsers = array(
        'webtv' => 'Web TV',
     ...
     *
     * @param mixed $config configuration file name or its contents parsed
     * @param string|bool $siteMemoryKey name of the application, used to create a virtual space in memory
     * @param string $env environment. used to retrieve configuration parameters
     *
     * @return Tecnodesign_App
     */
    public static function app($config, $siteMemoryKey = false, $env = 'prod')
    {
        if ($siteMemoryKey) {
            self::$_app = $siteMemoryKey;
            self::$_env = $env;
            Tecnodesign_Cache::siteKey($siteMemoryKey);
            if (!is_array($config) && file_exists($config)) {
                self::$timeout = filemtime($config);
                $cache = Tecnodesign_App::getInstance($siteMemoryKey, $env, self::$timeout);
                if ($cache) {
                    return $cache;
                }
            }
        }
        return new Tecnodesign_App($config, $siteMemoryKey, $env);
    }
    ...
    {
        return Tecnodesign_App::getInstance(self::$_app, self::$_env);
    }

    public static function appConfig()
    {
        /**
         * @todo Remove @ because it hides errors
         */
    ...
    public static function getUser()
    {
        static $className;
        if ($className === null) {
            $className = self::getApp()->config('user', 'className');
            if (!$className) {
                $className = 'Tecnodesign_User';
            }
        }
        return $className::getCurrent();
    }
    ...
    public static function connect($db = false, $app = null, $throw = false)
    {
        ...
        if (!is_array($db)) {
            ...
            if (self::$database === null) {
                ...
                if (!self::$database) {
                    if ($dbo = Tecnodesign_Cache::get('tdz/connect/' . $name, true)) {
                        $db = $dbo;
                        unset($dbo);
                    } elseif (file_exists($f = TDZ_APP_ROOT . '/config/databases.yml')) {
                        $C = Tecnodesign_Yaml::load($f);
                        self::$database = array();
                        if (isset($C[self::$_env])) {
                            self::$database = $C[self::$_env];
                        }
                        if (isset($C['all'])) {
                            self::$database += $C['all'];
                        }
            ...
            if (!is_array($db)) {
                if ($db && isset(self::$database[$db])) {
                    $db = self::$database[$db];
                } else {
                    $db = array_values(self::$database);
                    if (!isset($db['dsn'])) {
                        $db = array_shift($db);
                    }
                }
        ...
        $pgsql = false;
        if (isset($db['dsn'])) {
            if (strpos($db['dsn'], 'mysql:') === 0) {
                $mysql = true;
                $params['options'][PDO::MYSQL_ATTR_INIT_COMMAND] = 'SET NAMES utf8';
                $params['options'][PDO::ATTR_PERSISTENT] = true;
            } elseif (strpos($db['dsn'], 'sqlite:') === 0) {
                if (!isset($app)) {
                    $app = tdz::getApp();
                }
                $db['dsn'] = str_replace(array('$APPS_DIR', '$DATA_DIR'),
                    array($app->tecnodesign['apps-dir'], $app->tecnodesign['data-dir']), $db['dsn']);
            } elseif (strpos($db['dsn'], 'dblib:') === 0) {
                $mssql = true;
            }
        }
        $db += $params;
        ...
        try {
            ...
            if (isset($db['command'])) {
                if (!is_array($db['command'])) {
                    $db['command'] = array($db['command']);
                }
                foreach ($db['command'] as $q) {
                    @$conn->exec($q);
                }
            }
            self::$_connection[$name] = $conn;
            ...
            if (self::$connectRetries--) {
                self::log('retrying...');
                return self::connect($name, $app, $throw);
            }
            if ($throw) {
                throw new Tecnodesign_Exception(array(
                    self::t('Could not connect to database. Reasons are: %s', 'exception'),
                    $msg
                ));
            }
            return false;
        ...
        try {
            foreach ($sqls as $sql) {
                ...
                if ($query && count($arg) == 1) {
                    if (preg_match('/^\s*(insert|update|delete|replace|set|begin|commit|create|alter|drop) /i', $sql)) {
                        $result = true;
                    } else {
    ...
    public static function scriptName()
    {
        ...
        if (isset($a[0])) {
            ...
            if ($a[0] === true) {
                if (self::$real_script_name === null) {
                    if (isset($_SERVER['REDIRECT_STATUS'], $_SERVER['REDIRECT_URL'])
                        && $_SERVER['REDIRECT_STATUS'] === '200') {
                        self::$real_script_name = $_SERVER['REDIRECT_URL'];
                    } elseif (isset($_SERVER['REQUEST_URI'])) {
                        self::$real_script_name = $_SERVER['REQUEST_URI'];
                    } else {
                    ...
                    $queryStringPosition = strpos(self::$real_script_name, '?');
                    if ($queryStringPosition !== false) {
                        self::$real_script_name = substr(self::$real_script_name, 0, $queryStringPosition);
                    }
                }
                return self::$real_script_name;
            }

            if (is_string($a[0]) && strpos($a[0], '/') === 0) {
                $queryStringPosition = strpos($a[0], '?');
                if ($queryStringPosition !== false) {
                    $a[0] = substr($a[0], 0, $queryStringPosition);
                }

                self::$script_name = $a[0];
                if (isset($a[2]) && $a[2] === true) {
                    self::$real_script_name = $a[0];
                }
            }
        } elseif (self::$script_name === null) {
    ...
    public static function parseCss($s)
    {
        ...
        foreach ($css as $i => $r) {
            ...
            foreach ($ba as $a) {
                if (isset($css[$i][$a])) {
                    $bp = ($a == 'border') ? (array($css[$i][$a])) : (preg_split('/\s+/', $css[$i][$a], null,
                        PREG_SPLIT_NO_EMPTY));
                    $p = array();
                    if (count($bp) == 1) {
    ...
    public static function minify($s, $root = false, $compress = true, $before = true, $raw = false, $output = false)
    {
        ...
        $types = array(
            'js' => array(
                'pat' => '#<script [^>]*src="([^"\?\:]+)"[^>]*>\s*</script>#',
                'tpl' => '<script type="text/javascript"' . ((tdz::$async) ? (' async="async"') : ('')) . ' src="[[url]]"></script>'
            ),
            'css' => array(
                'pat' => '#<link [^>]*type="text/css"[^>]*href="([^"\?\:]+)"[^>]*>#',
                'tpl' => '<link rel="stylesheet" type="text/css" href="[[url]]" />'
            ),
        );
        ...
        if (is_array($s)) {
            ...
            foreach ($f as $i => $url) {
                ...
                $type = (substr($url, -5) == '.less' || substr($url, -4) == '.css' || strpos($url,
                        '.css?') !== false) ? ('css') : ('js');
                if (substr($url, 0, 1) != '/' && strpos($url, ':') === false) {
                    $url = tdz::$assetsUrl . '/' . $url;
                } else {
                    if ($output && is_string($output) && file_exists($url)) {
        ...
        foreach ($types as $type => $o) {
            ...
            if (count($files) > 0) {
                ...
                if ($output) {
                    ...
                    if (substr($root, 0, strlen(TDZ_ROOT)) == TDZ_ROOT) {
                ...
                foreach ($fs as $fk => $fv) {
                    ...
                    if (substr($fv, -5) == '.less') {
                        $css = $fv . '.css';
                        if (!file_exists($css)) {
                            $css = TDZ_VAR . '/min-' . md5($fv) . '-' . basename($fv);
                        }
                        if (!file_exists($css) || filemtime($css) < filemtime($fv)) {
                            ...
                            if (!$lc && class_exists('lessc')) {
                                ...
                                if ($root != TDZ_DOCUMENT_ROOT && is_dir($d = TDZ_DOCUMENT_ROOT . self::$assetsUrl . '/css/') && $imp[0] != $d) {
                                    array_unshift($imp, $d);
                                }
                                if (is_dir($d = $root . self::$assetsUrl . '/css/') && $imp[0] != $d) {
                                    array_unshift($imp, $d);
                                }
                                $lc->setImportDir($imp);
                ...
                if ($compress && $build) {
                    ...
                    if (isset(tdz::$minifier[$type])) {
                        ...
                        $cmd = tdz::$paths['cat'] . ' ' . implode(' ',
                                $fs) . ' | ' . tdz::$paths['java'] . ' -jar ' . dirname(TDZ_ROOT) . '/yuicompressor/yuicompressor.jar --nomunge --type ' . $type . ' -o ' . $tempnam;
                    }
                ...
                if ($raw) {
                    ...
                    $s = ($before) ? (str_replace('[[url]]', $url, $o['tpl']) . $s) : ($s . str_replace('[[url]]', $url,
                            $o['tpl']));
                }
            }
        }
    ...
    public static function render($d, $scope = null, $class = 'tdz-render', $translate = false, $xmlEscape = true)
    {
        ...
        if (is_object($d) && $d instanceof Tecnodesign_Model) {
            ...
            if (!$d) {
                $d = array_keys($cn::$schema['columns']);
            }
            $class .= ' caption';
        }
    ...
    public static function meta($s = '', $og = false)
    {
        ...
        if (!isset(tdz::$variables['meta'])) {
            ...
            if (is_array(tdz::$variables['meta'])) {
                tdz::$variables['meta'] = implode('', tdz::$variables['meta']);
            }
            if ($s) {
                tdz::$variables['meta'] .= $s;
            } else {
                if (isset(tdz::$variables['variables']['meta'])) {
        ...
        if ($og && !strpos(tdz::$variables['meta'], '<meta property="og:')) {
            tdz::$variables['meta'] .= tdz::openGraph();
        }
    ...
    public static function download(
        ...
        if ($format != '') {
            ...
            if ($fname) {
                $format = tdz::fileFormat($fname);
            } else {
                $format = tdz::fileFormat($file);
            }
            if ($format) {
                @header('content-type: ' . $format);
            }
        ...
        $gzip = false;
        if (substr($format, 0,
                5) == 'text/' && isset($_SERVER['HTTP_ACCEPT_ENCODING']) && substr_count($_SERVER['HTTP_ACCEPT_ENCODING'],
                'gzip')) {
            $gzip = true;
        }
    ...
    public static function serialize($a, $hint = null)
    {
        if (!is_null($hint)) {
            if ($hint == 'yaml') {
                return Tecnodesign_Yaml::dump($a);
            } else {
                if ($hint == 'json') {
    ...
    public static function unserialize($a, $hint = null)
    {
        if (is_string($a)) {
            if (!is_null($hint)) {
                if ($hint == 'yaml') {
                    return Tecnodesign_Yaml::load($a);
                } else {
                    if ($hint == 'json') {
            ...
            return (substr($a, 1, 1) == ':' && strpos('aOidsN',
                    substr($a, 0, 1)) !== false) ? (unserialize($a)) : (json_decode($a, true));
        }
    }
    ...
    {
        $s = html_entity_decode(preg_replace('/&([a-z]{1,2})(?:acute|cedil|circ|grave|lig|orn|ring|slash|th|tilde|uml);/i',
            '$1', htmlentities($s, ENT_QUOTES, 'UTF-8')), ENT_QUOTES, 'UTF-8');
        if ($accept) {
    ...
    public static function numberToLetter($i)
    {
        return tdz::letter($i);
    }
    ...
    {
        return tdz::bytes($bytes, $precision);
    }
    ...
    {
        return tdz::number($number, $decimals);
    }
    ...
    public static function safeHtml($s)
    {
        return preg_replace('#<(/?[a-z][a-z0-9\:\-]*)(\s|[a-z0-9\-\_]+\=("[^"]*"|\'[^\']*\')|[^>]*)*(/?)>#i', '<$1$2>',
            strip_tags($s, '<p><ul><li><ol><table><th><td><br><br/><div><strong><em><details><summary>'));
    }
        ...
        if ($alg) {
            ...
            if ($alg === 'uuid') {
                ...
                if ($r = Tecnodesign_Cache::get('uuid/' . $sh)) {
                    ...
                    while (!$r) {
                        $r = rtrim(strtr(base64_encode((function_exists('openssl_random_pseudo_bytes')) ? (openssl_random_pseudo_bytes(7)) : (pack('H*',
                            uniqid(true)))), '+/', '-_'), '=');
            ...
            if (is_string($type)) {
                ...
                if (substr($t, 0, 4) == 'SSHA' || substr(strtolower($t), 0, 4) == 'SMD5') {
                    if (is_null($salt)) {
                        ...
                        if (substr($salt, 0, strlen($type) + 2) == "{{$t}}") {
                            $salt = substr(base64_decode(substr($salt, strlen($type) + 2)),
                                strlen(hash(strtolower(substr($t, 1)), null, true)));
                        }
                    }
    ...
    public static function sameHost()
    {
        ...
        if (!isset($_SERVER['HTTP_REFERER']) || !($domain = preg_replace('/^https?\:\/\/([^\/]+).*/', '$1',
                $_SERVER['HTTP_REFERER'])) || $domain != $_SERVER['HTTP_HOST']) {
            return false;
        }
        return true;
    }
    ...
    public static function dateDiff($start, $end = '', $showtime = false)
    {
        ...
        if ($start == $end) {// same time
            ...
            if ($showtime && substr($start, 0, 10) == substr($end, 0, 10)) {// same day
                ...
                if (substr($start, 0, 7) == substr($end, 0, 7) && tdz::$dateFormat == 'd/m/Y') {// same month
                    $str = date('d', $tstart)
                        . ' ' . self::t('to', 'date')
                        . ' ' . self::date($tend, $showtime);
                } else {
                    if (substr($start, 0, 5) == substr($end, 0, 5) && tdz::$dateFormat == 'd/m/Y') { // same year
    ...
    public static function call($fn, $a = null)
    {
        if ($a) {
            if (!is_array($fn)) {
                return self::functionCall($fn, $a);
            } else {
                if (is_string($fn[0])) {
                    return self::staticCall($fn[0], $fn[1], $a);
                } else {
                    return self::objectCall($fn[0], $fn[1], $a);
                }
            }
        } else {
            if (!is_array($fn)) {
    ...
    {
        echo $question, ($check && is_array($check)) ? (' (' . implode(', ',
    ...
    {
        if ($filename = self::classFile($className)) {
            require_once $filename;
            self::autoloadParams($className);
        } elseif (self::$log > 10) {
            self::log(true, '[ERROR] Class ' . $className . ' was not found!');
        }
    ...
    {
        $c = str_replace(array('_', '\\'), '/', $className);
        $filename = TDZ_ROOT . "/src/{$c}.php";
        ...
        foreach (self::$lib as $libi => $d) {
            if (substr($d, -1) === '/') {
                self::$lib[$libi] = substr($d, 0, -1);
            }
            if (file_exists($filename = $d . '/' . $c . '.php')
                || file_exists($filename = $d . '/' . $c . '/' . $c . '.php')
                || file_exists($filename = $d . '/' . $c . '/' . $c . '.inc.php')
                || file_exists($filename = $d . '/' . $c . '.class.php')
                || file_exists($filename = $d . '/' . $c . '/' . strtolower($c) . '.php')
            ) {
                return $filename;
            }
            unset($libi, $d, $filename);
        }
    }

    public static function autoloadParams($className)
    {
        ...
        if (isset(self::$autoload[$className])) {
            foreach (self::$autoload[$className] as $k => $v) {
                $className::$$k = (!is_array($v) && strpos($v, '{') === 0) ? json_decode($v, JSON_OBJECT_AS_ARRAY) : $v;
                unset($k, $v);
            }
            unset(self::$autoload[$className]);
        }
        if (file_exists($c = TDZ_APP_ROOT . '/config/autoload.' . str_replace('\\', '_', $className) . '.ini')) {
            $c = parse_ini_file($c, true);
            if ($c) {
                foreach ($c as $k => $v) {
                    $className::$$k = self::rawValue($v);//(!is_array($v) && substr($v, 0, 1)=='{')?(json_decode($v, true)):($v);
                    unset($c[$k], $k, $v);
                }
            }
            unset($c);
        } elseif (file_exists($c = TDZ_APP_ROOT . '/config/autoload.' . str_replace('\\', '_', $className) . '.yml')) {
            $c = Tecnodesign_Yaml::load($c);
            if ($c) {
                foreach ($c as $k => $v) {
                    $className::$$k = self::rawValue($v);
                    unset($c[$k], $k, $v);
                }
            }
            unset($c);
        ...
        if (is_string($v)) {
            if ($v == 'true') {
                ...
                if ($v == 'false') {
                    ...
                    if (is_numeric($v) && preg_match('/^-?(0?\.[0-9]*|[1-9][0-9]*\.[0-9]*|0|[1-9][0-9]*)$/', $v)) {
...
if (TDZ_CLI && !file_exists(TDZ_VAR . '/no-install') && isset($_SERVER['argv'][1]) && ($_SERVER['argv'][1] == 'install' || (substr($_SERVER['argv'][1],
                0, 8) == 'install:') && isset(Tecnodesign_App_Install::$modules[substr($_SERVER['argv'][1], 8)]))) {
