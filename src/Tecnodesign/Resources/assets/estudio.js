if(!('tdz' in window)) {
    if('Z' in window) window.tdz = Z;
    else window.tdz={'started':true, 'exec':{} };
}
(function(tdz){

tdz.$=jQuery;
tdz.updated={};
tdz.cms_publish=function(f)
{
    f=$(f);
    f.attr('action',f.attr('action').replace(/\/edit\//,'/publish/'));
    tdz.cms_submit(f);
};
tdz.cms_unpublish=function(f)
{
    f=$(f);
    f.attr('action',f.attr('action').replace(/\/edit\//,'/unpublish/'));
    tdz.cms_submit(f);
};
...
function startup(){
    console.log('estudio!!!');
    if(tdz.feed!='' && $('.tdzs').length > 0 && $('#tdzcms').length==0){
      tdz.update_slots();
      if(tdz.poll){
        tdz.interval=setInterval('tdz.update_slots();',tdz.poll);
      };
    };
};
