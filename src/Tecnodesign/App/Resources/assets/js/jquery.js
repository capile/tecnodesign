    ...
    $.fn.extend({
        ...
        mask: function(mask, settings) {
            ...
            if (arguments.length==0 && this.length > 0) {
                if(this.length==1 && this.attr('data-mask')) {
                    mask = this.attr('data-mask');
                } else {
                    this.each(function(i,o){
                        o=$(o);if(o.attr('data-mask'))
...
(function($) {
		...
		_updateDateTime: function(dp_inst) {
			...
			*/
			...
			if (!this._defaults.showTimepicker) {
				...
				this.$input.val(formattedDateTime);
				if(p!=formattedDateTime && this.$input.val()==p) {
					//this.$input.val(this.formattedDate+'T'+this.formattedTime+this._defaults.timeSuffix);
					//this._defaults.separator = 'T';
					this.$input.attr('value', dt.toISOString().replace(/\.[0-9]+Z$/, ''));
	...
	*/
	...
	$.datepicker.parseDate = function(format, value, settings) {
		var date;
		// the W3 way is iso, which is unsupported by Datepicker
		if(value && value.indexOf(/^[0-9]{4}\-[0-9]{2}\-[0-9]{2}T/)>-1) {
			date = new Date(value);
			// change from datetime to datetime-local -- should check the input
	...
	*/
	var splitDateTime = function(dateFormat, dateTimeString, dateSettings, timeSettings) {
		if(dateTimeString.search(/^[0-9]{4}\-[0-9]{2}\-[0-9]{2}T/)>-1) {
			return [ dateTimeString.substr(0,10), dateTimeString.substr(11,5) ];
		}
		try {
	...
	*/
	...
	$.timepicker.log = function(err){
		if(window.console){
			//console.log(err); // many errors on chrome due to no support for datetime
		}
	};
