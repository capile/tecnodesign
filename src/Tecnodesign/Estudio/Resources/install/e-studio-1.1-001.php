...
if(!class_exists('tdz')) {
    ...
    if(Tecnodesign_Estudio::VERSION<1.1) return false;
}
$cid = tdz::getApp()->estudio['connection'];
...
"create table tdz_entries (
  ...
  primary key (id)
    ...
    if($driver!='sqlite') $q[0] .= "comment = 'className: Tecnodesign_Estudio_Entry'";
    tdz::query($q);
}
...
"create table tdz_contents (
  ...
  position bigint(20) null default null{$comment_sortable},
  published datetime null default null,
  ...
  'create index tdz_contents_updated_idx on tdz_contents(updated asc)',
    ...
    if($driver!='sqlite') $q[0] .= "comment = 'className: Tecnodesign_Estudio_Content'";
    tdz::query($q);
}
...
"create table tdz_permissions (
  ...
  'create index tdz_permissions_role_idx on tdz_permissions(role asc)',
    ...
    if($driver!='sqlite') $q[0] .= "comment = 'className: Tecnodesign_Estudio_Permission'";
    tdz::query($q);
}
...
"create table tdz_relations (
  ...
  'create index tdz_relations_entry_idx on tdz_relations(entry asc)',
    ...
    if($driver!='sqlite') $q[0] .= "comment = 'className: Tecnodesign_Estudio_Relation'";
    tdz::query($q);
}
...
"create table tdz_tags (
  ...
  'create index tdz_tags_slug_idx on tdz_tags(slug asc)',
    ...
    if($driver!='sqlite') $q[0] .= "comment = 'className: Tecnodesign_Estudio_Tag'";
    tdz::query($q);
}
...
"create table tdz_contents_display (
  ...
  constraint fk_tdz_contents_display__content
    ...
    if($driver!='sqlite') $q[0] .= "comment = 'className: Tecnodesign_Estudio_ContentDisplay'";
    tdz::query($q);

    // upgrade from previous estudio versions, migrate column tdz_contents.show_at|hide_at to this table
...
"create table tdz_groups (
  ...
  'create index tdz_groups_priority_idx on tdz_groups(priority asc)',
    ...
    if($driver!='sqlite') $q[0] .= "comment = 'className: Tecnodesign_Estudio_Group'";
    tdz::query($q);
}
...
"create table tdz_users (
  ...
  'create index login_idx on tdz_users(login asc)',
    ...
    if($driver!='sqlite') $q[0] .= "comment = 'className: Tecnodesign_Estudio_User'";
    tdz::query($q);
}
...
"create table tdz_credentials (
  ...
  'create index groupid_idx on tdz_credentials(groupid asc)',
    ...
    if($driver!='sqlite') $q[0] .= "comment = 'className: Tecnodesign_Estudio_Credential'";
    tdz::query($q);
}