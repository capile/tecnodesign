...
class Tecnodesign_Studio
{
        ...
        if (self::$automatedInstall) {
            ...
            if ($version < self::VERSION) {
                if ($version < self::VERSION) {
                    ...
                    tdz::save($if,
                        Tecnodesign_Yaml::dump(array('id' => 'Tecnodesign_Studio', 'version' => self::VERSION)));
                }
    ...
    {
        self::$app = tdz::getApp();
        $request = Tecnodesign_App::request();
        if ($request['shell']) {
            tdz::scriptName($request['script-name']);
            self::$cacheTimeout = false;
            self::$staticCache = false;
            /**
             * @todo This should be avoided. Use a global constant or fix the autoloader
             */
            chdir(TDZ_APP_ROOT);
        }
        $scriptName = $request['script-name'];

        if (!self::$languages && isset(self::$app->tecnodesign['languages'])) {
            self::$languages = self::$app->tecnodesign['languages'];
        }

        if (self::$languages) {
            /**
             * @todo this method is not static, ->language should be used instead
             */
            tdz::$lang = self::language(self::$languages);
        }

        if ((self::$connection === null) && isset(self::$app->studio['connection'])) {
        ...
        if (isset($_GET['!rev'])) {
            ...
            self::$staticCache = 0;
        } elseif (self::$cacheTimeout && self::$staticCache) {
            self::getStaticCache();
        }
        ...
        // try the interface
        if ($scriptName === self::$home || strncmp($scriptName, self::$home, strlen(self::$home)) === 0) {
            if ($scriptName !== self::$home) {
                tdz::scriptName($scriptName);
            }
            return self::_runInterface();
        }

        if (isset($_SERVER['HTTP_TDZ_SLOTS']) || $scriptName === self::$uid) {
            tdz::cacheControl('private', static::$cacheTimeout);
            tdz::output(json_encode(self::uid()), 'json');
        } elseif (self::ignore($scriptName)) {
            self::error(404);
        } elseif ($tdzEntry = self::page($scriptName)) {
            $tdzEntry->render();
            if (!self::$private && !tdz::get('cache-control')) {
                tdz::cacheControl('public', 600);
            }
            return true;
        } elseif (strpos($scriptName, tdz::$assetsUrl) === 0 && tdzAsset::run($scriptName)) {
            return true;
        } elseif (TDZ_CLI && $scriptName === 'studio') {
    ...
    public function language($l = array())
    {
        if (!is_array($l)) {
            ...
            if (count($l) < 2) {
                ...
                if (substr($req['query-string'], 0, 1) == '!' && in_array($lang = substr($req['query-string'], 1),
                        $l)) {
                    setcookie('lang', $lang, 0, '/', false, false);
                    tdz::redirect($req['script-name'] . '?' . $lang);
                }
                unset($lang);
                if (!(isset($_COOKIE['lang']) && ($lang = $_COOKIE['lang']) && (in_array($lang,
                            $l) || (strlen($lang) > 2 && in_array($lang = substr($lang, 0, 2), $l))))) {
                    unset($lang);
                }
                if (!isset($lang) && isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
                    $accept = preg_split('/(;q=[0-9\.]+|\,)\s*/', $_SERVER['HTTP_ACCEPT_LANGUAGE'], null,
                        PREG_SPLIT_NO_EMPTY);
    ...
    private static function _runInterface()
    {
        ...
        if (strpos($url, '.') !== false) {
            ...
            if (!($U = tdz::getUser()) || !$U->isAuthenticated() || !($U->isSuperAdmin() || ($c = self::credential(array(
                        'studio',
                        'edit',
                        'previewUnpublished'
                    ))) && $U->hasCredential($c, false))) {
    ...
    public static function listInterfaces()
    {
        ...
        if (!$p) {
            $p = Tecnodesign_App::request('get', 'q');
        }
    ...
    public static function listProperties()
    {
        ...
        $C = array(
            ...
            'newTemplate' => ($U->isSuperAdmin() || ($c = self::credential('newTemplate')) && $U->hasCredential($c,
                    false)),
            'edit' => ($U->isSuperAdmin() || ($c = self::credential('edit')) && $U->hasCredential($c, false)),
            'editTemplate' => ($U->isSuperAdmin() || ($c = self::credential('editTemplate')) && $U->hasCredential($c,
                    false)),
        ...
        if ($C['edit'] && ($d = Tecnodesign_App::request('post', 'c'))) {
            foreach ($d as $i => $id) {
                ...
                if ($o && $o->pageFile && substr(basename($o->pageFile), 0, 6) == '_tpl_.' && !$C['editTemplate']) {
                    $o = null;
                }
                ...
                if ($o->pageFile && substr($o->pageFile, 0, strlen(tdzEntry::$pageDir)) == tdzEntry::$pageDir) {
                    $R[$k]['id'] = substr($o->pageFile, strlen(tdzEntry::$pageDir));
                }
                unset($d[$i], $id, $k, $o);
            }
        }
    ...
    public static function content($page, $checkLang = true, $checkTemplates = true)
    {
        ...
        if (!isset(tdzContent::$contentType[$ext])) {
            return false;
        } else {
            if (is_array(tdzContent::$disableExtensions) && in_array($ext, tdzContent::$disableExtensions)) {
                return false;
            }
        }
        ...
        if ($m) {
            ...
            if (isset($meta['credential'])) {
                ...
                if (!is_array(Tecnodesign_Studio::$private)) {
                    Tecnodesign_Studio::$private = $c;
                } else {
                    Tecnodesign_Studio::$private = array_merge($c, Tecnodesign_Studio::$private);
                }
                unset($U);
            }
        }
        ...
        if (isset($meta) && $meta) {
            $C->_meta = $meta;
            if (isset($meta['credential'])) {
                unset($meta['cdredential']);
            }
        }
        if (!is_null($pos)) {
            $C->_position = $slotname . $pos;
        }
        if (isset($meta)) {
    ...
    public static function getStaticCache()
    {
        ...
        if ($r && is_array($r)) {
            if (!isset($r['h']['Content-Type'])) {
                $r['h']['Content-Type'] = 'text/html;charset=UTF8';
            }
            if (is_array($r['c'])) {
    ...
    public static function uid()
    {
        ...
        if (Tecnodesign_Studio::$checkOrigin) {
            ...
            if (isset($_SERVER['HTTP_ORIGIN'])) {
                ...
                if (isset($_SERVER['HTTP_REFERER'])) {
                    ...
                    if (Tecnodesign_Studio::$checkOrigin > 1) {
                        return false;
                    }
                }
            }
        ...
        if ($U) {
            $r = $U->asArray();
            if (self::$webInterface && (($U->isAuthenticated() && ($U->isSuperAdmin()) || (($ec = self::credential('studio')) && $U->hasCredential($ec,
                            false))))) {
                $r['plugins'] = array(
    ...
    public static function page($url, $exact = false, $published = null)
    {
        ...
        if ($published === null) {
            ...
            if (self::$connection && ($U = tdz::getUser()) && $U->hasCredential($c = self::credential('previewUnpublished'),
                    false)) {
                $published = false;
                ...
                if (isset(self::$params['!rev'])) {
                    ...
                    if (substr(tdzEntry::$schema['table_name'], -8) !== '_version') {
                        tdzEntry::$schema['table_name'] .= '_version';
                    }
                    if (substr(tdzContent::$schema['table_name'], -8) !== '_version') {
                        tdzContent::$schema['table_name'] .= '_version';
                    }
                    self::$cacheTimeout = false;
                }
            } else {
        ...
        */
        ...
        $tdzEntity = null;
        if (self::$connection && $tdzEntity = tdzEntry::find($f, 1, $scope, false,
                array('type' => 'desc', 'version' => 'desc'))) {
            return $tdzEntity;
        }

        if (!$tdzEntity && ($tdzEntity = tdzEntry::findPage($url, false, true))) {
            if (!self::$connection) {
                self::checkIndex();
            }
        } elseif (preg_match('/(' . str_replace('.', '\.', implode('|', self::$allowedExtensions)) . ')$/', $url,$matches)
            && ($tdzEntity = tdzEntry::findPage(substr($url, 0, strlen($url) - strlen($matches[1])), false, true))) {
            $url = substr($url, 0, strlen($url) - strlen($matches[1]));
            if (!self::$connection) {
                self::checkIndex();
            }
        }

        if (!$tdzEntity && !$exact && strpos($url, '/') === 0 && strlen($url) > 1) {
            $f['Content.content_type'] = tdzContent::$multiviewContentType;
            $u = $url;
            while (strlen($u) > 1) {
                $u = preg_replace('#/[^/]+$#', '', $u);
                $f['link'] = $u;
                if (self::$connection && ($tdzEntity = tdzEntry::find($f, 1, $scope, false, array('type' => 'desc')))) {
                    break;
                }

                if ($u && ($tdzEntity = tdzEntry::findPage($u, true))) {
                    break;
                }
            }
    ...
    {
        $tdzEntry = new tdzEntry(array('link' => $url), false, false);
        $relatedContent = $tdzEntry->getRelatedContent();
        unset($tdzEntry);
        $tpl = array();
        if ($relatedContent) {
            foreach ($relatedContent as $i => $o) {
                if (!isset($tpl[$o->slot])) {
                    $tpl[$o->slot] = '';
                }
                $r = $o->render();
                $tpl[$o->slot] .= $o->render();
                unset($relatedContent[$i], $o, $i);
            }
    ...
    {
        if (!Tecnodesign_Studio::$app) {
            Tecnodesign_Studio::$app = tdz::getApp();
        }
    ...
    public static function credential($s)
    {
        if (is_null(self::$credentials)) {
            ...
            if (!self::$credentials && self::$cacheTimeout) {
                self::$credentials = Tecnodesign_Cache::get('studio/credentials', self::$cacheTimeout);
            }
            if (!self::$credentials || !is_array(self::$credentials)) {
                self::$credentials = array();
                if (self::$connection) {
                    $ps = tdzPermission::find(array('entry' => ''), 0, array('role', 'credentials'), false,
                        array('updated' => 'desc'));
                    if ($ps) {
                        foreach ($ps as $i => $P) {
                            if (isset(self::$credentials[$P->role])) {
                                continue;
                            }
                            if (!$P->credentials || $P->credentials == '~') {
                                ...
                                if ($P->credentials == '*') {
                                    ...
                                    $c = preg_split('/[\s\,\;]+/', $P->credentials, null, PREG_SPLIT_NO_EMPTY);
                                }
                            }
                ...
                if (self::$cacheTimeout) {
                    Tecnodesign_Cache::set('studio/credentials', self::$credentials, self::$cacheTimeout);
                }
            }
        }
        if (!self::$credentials) {
    ...
    public static function templateFile($tpl)
    {
        ...
        $template = false;
        /**
         * @todo foreach is overwriting params!
         */
        foreach (func_get_args() as $tpl) {
            if ($tpl && (
                (strpos($tpl, $apps) === 0 && file_exists($tplFile = $tpl . '.php'))
                || file_exists($tplFile = $tpld . '/' . $tpl . '.php')
                || file_exists($tplFile = TDZ_ROOT . '/src/Tecnodesign/Studio/Resources/templates/' . $tpl . '.php')
            )) {
                $template = $tplFile;
                break;
            }
    ...
    public static function t($s, $alt = null, $table = 'lang')
    {
        ...
        if (!isset($T[$table])) {
            ...
            if (!$T[$table]) {
                ...
                if (self::$cacheTimeout) {
                    Tecnodesign_Cache::set($ckey, $T[$table], self::$cacheTimeout);
                }
            }
            unset($lang, $ckey);
        }
    ...
    {
        $s = '';
        if ($list instanceof Tecnodesign_Collection) {
            $list = $list->getItems();
        }
        ...
        foreach ($toAdd as $k) {
            if (isset($a[$k])) {
                if (!isset(tdz::$variables[$k])) {
                    ...
                    if (!is_array($a[$k])) {
                        if (!in_array($a[$k], tdz::$variables[$k])) {
                            tdz::$variables[$k][] = $a[$k];
                        }
                    } else {
...
if (!class_exists('tdzEntry')) {
    if (!in_array($libdir = dirname(__FILE__) . '/Studio/Resources/model', tdz::$lib)) {
        tdz::$lib[] = $libdir;
    }
    unset($libdir);
}

if (!defined('TDZ_ESTUDIO')) {
    define('TDZ_ESTUDIO', Tecnodesign_Studio::VERSION);
}
